...
export class FilesService {
    ...
    async createFile(  file: any, s3url: string, nameFile: string, usuario_id:number ){
    ...
    async createMultipleFiles(files: any[], namesFiles: string[], usuario_id: number){
      for(let i = 0; i < files.length; i++) {
            const file = files[i];
            const s3url  = await this.storageService.upload(file);
            console.log(namesFiles[i]);
