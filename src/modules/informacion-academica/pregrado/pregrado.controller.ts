...
export class PregradoController {
  constructor(private readonly pregradoService: PregradoService) {}
  ...
  @ApiOperation({
    description:
      'Devuelve todos las informaciones academica de pregrado por paginacion de una facultad ',
  })
  @ApiParam({
    name: 'slug',
    type: String,
    required: true,
    description:
      'SLUG de la facultad a la que pertenecen las informaciones academicas de pregrado',
  })
  ...
  async createPregrado(@Body() dto: CreatePregradoDto, @Res() response) {
    try {
  ...
  async EditPregradoDto(@Param('id') id: number, @Body() dto: EditPregradoDto) {
    let data;
